    ...
    if (this->getValue() == "=")
    {
        ...
        if (this->children.size() == 2)
        {
            ...
            if (record_ptr)
            {
                ...
                if (lhs_record_type != rhs_type
                        && rhs_type.find(lhs_record_type) == string::npos) // extends
                {
                    cerr << "[Semantic Analysis] - Error: Can not assign \""
                         << rhs_type << "\" (rhs) to \"" << lhs_record_type << "\" (lhs) in the scope \""
            ...
            {
                cerr << "[Semantic Analysis] - Error: Variable \"" << lhs_name
                     << "\" does not exist in scope \"" << Statement::st.getScopeTitle()
                     << "\"!" << endl;
                exit(EXIT_FAILURE);
            }
        }
